...
// copied from https://github.com/moment/luxon/issues/1134#issuecomment-1282092129
...
;(Duration.prototype as any).toHuman = function (
    ...
    let durationUnits = []
    let precision = typeof opts.precision == "object" ? Duration.fromObject(opts.precision) : Duration.fromMillis(0)
    ...
    if (!durationUnits.length) {
        // @ts-expect-error idk just ship it
